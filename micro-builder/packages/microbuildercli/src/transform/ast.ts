...
export function toCss(ast: AstNode[]) {
  ...
  function stringify(node: AstNode, depth = 0): string {
    ...
    if (node.kind === "rule") {
      ...
      if (
        node.selector[0] === "@" &&
        node.selector.startsWith("@property ") &&
        depth === 0
      ) {
        ...
        if (inherits) {
          ...
          propertyFallbacksUniversal.push(
            decl(property, initialValue ?? "initial")
          );
        }
    ...
    // Declaration
      ...
      node.property !== "--tw-sort" &&
      node.value !== undefined &&
      node.value !== null
    ) {
      css += `${indent}${node.property}: ${node.value}${
  ...
  if (propertyFallbacksUniversal.length) {
    fallbackAst.push(
      rule("*, ::before, ::after, ::backdrop", propertyFallbacksUniversal)
    );
  }
  ...
  if (fallbackAst.length) {
    fallback = stringify(
      rule("@supports (-moz-orient: inline)", [
